...
Training logs full move by move detail for sampled games, controlled by
`--event-every`, and each of those is written as a replay with a six character id
such as `2ZH4P5`. The id appears in `games.jsonl`, in `games.csv`, and in the
dropdown on the viewer page.

`docs\index.html` is the viewer: a board that plays the game back move by move,
with tokens, ownership marks, houses and hotels, live cash, and a clickable move
log. Paste an id, pick one from the recent list, or open a replay file saved on your
device. `?g=2ZH4P5` on the URL loads one directly, which makes replays shareable.

Locally:

```powershell
python run.py demo --seed 7
python run.py replays
python -m http.server --directory docs 8000
```

Then open `http://localhost:8000`. Opening the file directly from disk will not work
for the id box, because browsers block local fetches; the Open file button still
does. On GitHub, turn on Pages under Settings, Pages, source Deploy from a branch,
branch main, folder /docs. The training workflow refreshes the replays and the page
on every run, keeping the most recent 150 games.
